...
describe('FLAT_V1 Anti Snipe curve token', () => {
  let moonit: Moonit;

  const rpcUrl = process.env.RPC_URL as string;
  const mintAddress = process.env.FLAT_CURVE_ANTI_SNIPE_MINT as string;
  ...
  test('should get the token details from curve account', async () => {
    const priceFor25SolCollateral = 51n;
    const constantPrice = await token.getCollateralPrice({
      tokenAmount: BigInt(1e9), // 1 token in minimal units
      curvePosition: 0n,
    });
    expect(constantPrice).toBe(priceFor25SolCollateral);

    const curveAccount = await token.getCurveAccount();

    expect(curveAccount.curveType).toBe(4);
    expect(curveAccount.marketcapThreshold).toBe(25000000000n);
